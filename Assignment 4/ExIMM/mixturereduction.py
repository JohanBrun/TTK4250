...
def gaussian_mixture_moments(
    ...
    # mean
    mean_bar = np.average(mean, axis=0, w)  # TODO: hint np.average using axis and weights argument

    # covariance
    # # internal covariance
    cov_int = np.average(cov, axis=0, w)  # TODO: hint, also an average

    # # spread of means
    # Optional calc: mean_diff =
    cov_ext = np.average(mean @ mean.T, axis=0, w) + mean_bar @ mean_bar.T  # TODO: hint, also an average
